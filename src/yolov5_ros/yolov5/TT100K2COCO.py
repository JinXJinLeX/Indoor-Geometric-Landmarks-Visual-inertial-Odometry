...
class TT100K2COCO:
    ...
    #######################################################
    #对TT100K中的数据集进行预处理，代码中是只保留包含图片数超过100的类别；
    def class_statistics(self):
        # os.makedirs('annotations', exist_ok=True)
        # 存放数据的父路径
        #parent_path = 'D:/jinxData/TT100K/data'
        parent_path = '/media/scott/Studio/z/TT100K2016/data'
    ...
    def json2xml(self):
        img_path = '/media/scott/Studio/z/TT100K2016/data/train/'  # train图片路径parent_path = '/media/scott/Studio/z/TT100K2016/data'
        annos = json.loads(open("/media/scott/Studio/z/TT100K2016/data/annotations.json").read())
        xml_path = '/media/scott/Studio/z/TT100K2016/data/xml_train/'  # xml保存路径
...
#######################################################
    #将CoCo格式的标签json文件（3个json文件）转换为YOLO格式的txt（注意此处需要分别调用三次，train、val和test，对应生成其3个.txt文件）；
    def coco_json2yolo_txt(self, class_json):
        ...
        json_file = os.path.join(
            '/media/scott/Studio/z/TT100K2016/data/dataset/annotations/%s.json' % class_json)  # COCO Object Instance 类型的标注
        # ana_txt_save_path = 'D:/jinxData/TT100K/data/dataset/annotations/train'  # 保存的路径
        ana_txt_save_path = os.path.join('/media/scott/Studio/z/TT100K2016/data/dataset/annotations/train', class_json)  # 保存的路径
    ...
    def divide_TrainValTest(self, source, target):
        ...
        for root, dirs, files in os.walk(source):
            for file in files:
                ...
                # print(source)
                if 'train' in source:
                    shutil.copyfile('/media/scott/Studio/z/TT100K2016/data/image_reparation/'
...
#######################################################
    #根据重新划分的结果对原始数据集的图像进行重新划分存储为与标签一一对应的格式，这一步结束即可将重新规划后的数据集以及标签文件整理为数据集通过YOLO5进行训练预测。
    tt100k.divide_TrainValTest('/media/scott/Studio/z/TT100K2016/data/dataset/annotations', '/media/scott/Studio/z/TT100K2016/data/dataset')
    tt100k.divide_TrainValTest('/media/scott/Studio/z/TT100K2016/data/dataset/annotations', '/media/scott/Studio/z/TT100K2016/data/dataset')
    tt100k.divide_TrainValTest('/media/scott/Studio/z/TT100K2016/data/dataset/annotations', '/media/scott/Studio/z/TT100K2016/data/dataset')
