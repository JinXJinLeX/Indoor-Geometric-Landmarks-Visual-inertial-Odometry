				   ...
				   vector<double> &_point_id, int _sequence)
{
	...
	// 20240917 xjl
	sign_loop_info << 0, 0, 0, 0, 0, 0, 0, 0;
	has_sign_loop = false;
...
KeyFrame::KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, Vector3d &_T_w_i, Matrix3d &_R_w_i,
				   cv::Mat &_image, int _loop_index, Eigen::Matrix<double, 8, 1> &_loop_info, int _sign_loop_index, Eigen::Matrix<double, 8, 1> &_sign_loop_info,
				   vector<cv::KeyPoint> &_keypoints, vector<cv::KeyPoint> &_keypoints_norm, vector<BRIEF::bitset> &_brief_descriptors)
{
	...
	// 20240919 xjl
	if (_sign_loop_index != 1)
		has_sign_loop = true;
	else
		has_sign_loop = false;
	sign_loop_info = _sign_loop_info;
}
...
// 20240917 xjl
Eigen::Vector3d KeyFrame::getSignLoopRelativeT()
{
	return Eigen::Vector3d(sign_loop_info(0), sign_loop_info(1), sign_loop_info(2));
}

Eigen::Quaterniond KeyFrame::getSignLoopRelativeQ()
{
	return Eigen::Quaterniond(sign_loop_info(3), sign_loop_info(4), sign_loop_info(5), sign_loop_info(6));
}

double KeyFrame::getSignLoopRelativeYaw()
{
	return sign_loop_info(7);
}

void KeyFrame::updateSignLoop(Eigen::Matrix<double, 8, 1> &_sign_loop_info)
{
