...
// 20240917 xjl
void sign_relative_pose_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    static Vector3d sign_last_t;
    Vector3d sign_relative_t = Vector3d(pose_msg->pose.pose.position.x,
                                        pose_msg->pose.pose.position.y,
                                        pose_msg->pose.pose.position.z);
    Quaterniond sign_relative_q;
    sign_relative_q.w() = pose_msg->pose.pose.orientation.w;
    sign_relative_q.x() = pose_msg->pose.pose.orientation.x;
    sign_relative_q.y() = pose_msg->pose.pose.orientation.y;
    sign_relative_q.z() = pose_msg->pose.pose.orientation.z;
    double sign_relative_yaw = pose_msg->twist.twist.linear.x;
    int index = pose_msg->twist.twist.linear.y;
    if ((sign_last_t - sign_relative_t).norm() < 0.01)
        return;
    sign_last_t = sign_relative_t;
    cout << "relative_t:" << sign_relative_t.x() << "," << sign_relative_t.y() << "," << sign_relative_t.z() << endl;
    cout << "relative_q:" << sign_relative_q.w() << "," << sign_relative_q.x() << "," << sign_relative_q.y() << "," << sign_relative_q.z() << endl;
    if ((sign_relative_t.x()*sign_relative_t.x()) < 0.01 && (sign_relative_t.y()*sign_relative_t.y()) < 0.01 && (sign_relative_t.z()*sign_relative_t.z()) < 0.01)
    {
        return;
    }
    ...
    sign_loop_info << sign_relative_t.x(), sign_relative_t.y(), sign_relative_t.z(),
        ...
        sign_relative_yaw; // 相对位姿变换
    double time_stamp = pose_msg->header.stamp.toSec();
    posegraph.updateKeyFrameSignLoop(time_stamp, sign_loop_info);
}

void process()
{
    if (!LOOP_CLOSURE && !SIGN_CLOSURE) // 如果没有回环检测 直接return
        return;
    while (true)
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            if ((T - last_t).norm() > SKIP_DIS)
            {
                ...
                m_process.lock();
                start_flag = 1;
                posegraph.addKeyFrame(keyframe, 1); // 加入关键帧
                m_process.unlock();
                frame_index++;
                last_t = T;
            }
...
int main(int argc, char **argv)
{
    ...
    SIGN_CLOSURE = fsSettings["sign_closure"];
    ...
    if (LOOP_CLOSURE || SIGN_CLOSURE)
    {
        ...
        // VINS_RESULT_PATH = VINS_RESULT_PATH + "/vins_sign_loop.txt";
    ...
    // 20240917 xjl
    ros::Subscriber sub_sign_relative_pose = n.subscribe("/vins_estimator/relo_sign_pose", 2000, sign_relative_pose_callback); // 接收标志引起的位姿变化话题
