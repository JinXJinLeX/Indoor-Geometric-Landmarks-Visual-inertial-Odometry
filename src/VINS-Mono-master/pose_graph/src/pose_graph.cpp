...
PoseGraph::PoseGraph()
{
    ...
    // 20240918 xjl
    // sign_optimization = std::thread(&PoseGraph::optimizeSign4DoF, this);
    ...
    // 20240918 xjl
    sign_t_drift = Eigen::Vector3d(0, 0, 0);
    sign_yaw_drift = 0;
    sign_r_drift = Eigen::Matrix3d::Identity();
...
void PoseGraph::addKeyFrame(KeyFrame *cur_kf, bool flag_detect_loop)
{
    ...
    {
        TicToc tmp_t;
        loop_index = detectLoop(cur_kf, cur_kf->index); // 用当前帧和以前的帧触发回环，得到索引
    }
    else
    {
...
KeyFrame *PoseGraph::SigngetKeyFrame(double time_stamp)
{
    list<KeyFrame *>::iterator it = keyframelist.begin();
    double pre, cur;
    for (; it != keyframelist.end(); it++)
...
// 20240918 xjl
// void PoseGraph::optimizeSign4DoF()
// {
//     int cur_index = -1;
//     int pre_index = -1;
//     // judge sign_optimize_buf
//     while (true)
//     {
//         if (sign_optimize_buf.empty())
//         {
//             cout << "???" << endl;
//             continue;
//         }
//         m_sign_optimize_buf.lock();
//         if (!sign_optimize_buf.empty())
//         {
//             pre_index = cur_index;
//             cur_index = sign_optimize_buf.front();
//             sign_optimize_buf.pop();
...
//             {
//                 if ((*it)->index < cur_index - 15 || (*it)->index > cur_index - 1)
//                     continue;
...
//                 cout << "1==================" << endl;
//                 if ((*it)->index == cur_index - 15 || (*it)->index < 2)
//                 {
...
//                 //     Vector3d relative_t;
//                 //     relative_t = (*it)->getSignLoopRelativeT();
//                 //     double relative_yaw = (*it)->getSignLoopRelativeYaw();
...
//             m_sign_drift.lock();
//             sign_yaw_drift = Utility::R2ypr(cur_r).x() - Utility::R2ypr(vio_r).x();
//             sign_r_drift = Utility::ypr2R(Vector3d(yaw_drift, 0, 0));
//             sign_t_drift = cur_t - sign_r_drift * vio_t;
//             m_sign_drift.unlock();
...
//             it++;
//             for (; it != keyframelist.end(); it++) // update untill end
//             {
//                 Vector3d P;
//                 Matrix3d R;
//                 (*it)->getVioPose(P, R);
//                 P = sign_r_drift * P + sign_t_drift;
//                 R = sign_r_drift * R;
//                 (*it)->updatePose(P, R);
//             }
//             m_keyframelist.unlock();
...
void PoseGraph::optimize4DoF()
{
    while (true)
    {
        ...
        // 20241020_xjl
        // int cur_index = -1;
        static int pre_sign_index = -1;
        int sign_index = -1;
        m_sign_optimize_buf.lock();
        if (!sign_optimize_buf.empty())
        {
            pre_sign_index = cur_index;
            cur_index = sign_optimize_buf.front();
            sign_optimize_buf.pop();
            while (cur_index == sign_optimize_buf.front())
            {
                sign_optimize_buf.pop();
            }
            if (pre_sign_index == cur_index)
            {
                continue;
            }
        }
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                // 20241021_xjl
                // printf("last_index:%d\n", last_index);
                // if ((*it)->index <= max(last_index - 100, last_last_index))
                // {
                //     problem.SetParameterBlockConstant(euler_array[i]);
                //     problem.SetParameterBlockConstant(t_array[i]);
                // }
                ...
                //     sign_index = (*it)->index;
                //     // printf("sign optimize pose graph \n");
                //     cout << " (*it)->index:" << (*it)->index << endl;
                //     // cout << "begin_index:" << last_index << endl;
                //     cout << "t_array:" << t_array[last_index][0] << "," << t_array[last_index][1] << "," << t_array[last_index][2] << endl;
                //     // cout << "q_array:" << q_array[last_index].x() << "," << q_array[last_index].y() << "," << q_array[last_index].z() << "," << q_array[last_index].w() << endl;
                //     // double res = ((t_array[(*it)->idex][0] - t_array[last_index][0]) * (t_array[(*it)->index][0] - t_array[last_index][0]) +
                //     //               (t_array[(*it)->index][1] - t_array[last_index][1]) * (t_array[(*it)->index][1] - t_array[last_index][1]) +
                //     //               (t_array[(*it)->index][2] - t_array[last_index][2]) * (t_array[(*it)->index][2] - t_array[last_index][2]));
                //     // if (res > 0.1)
                //     {
                //         Vector3d euler_conncected = Utility::R2ypr(q_array[last_index].toRotationMatrix());
                //         Vector3d relative_t;
                //         relative_t = (*it)->getSignLoopRelativeT();
                //         double relative_yaw = (*it)->getSignLoopRelativeYaw();
                //         ceres::CostFunction *cost_function = FourDOFWeightError_Sign::Create(relative_t.x(), relative_t.y(), relative_t.z(), t_array[last_index][0], t_array[last_index][1], t_array[last_index][2],
                //                                                                              relative_yaw, euler_array[last_index][0], euler_array[last_index][1], euler_array[last_index][2]);
                //         problem.AddResidualBlock(cost_function, loss_function, t_array[last_index]);
                //     }
                //     (*it)->has_sign_loop = false;
                //     // break;
                // }
                if ((*it)->index == cur_index)
                    break;
            ...
            */
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                if ((*it)->index < first_looped_index)
                    continue;
                // if ((*it)->index >= last_index)
                // {
                //     continue;
                // }
                // if ((*it)->index <= last_last_index)
                // {
                //     continue;
                // }
                Quaterniond tmp_q;
...
void PoseGraph::loadPoseGraph()
{
    ...
    // 20240917 xjl
    double sign_loop_info_0, sign_loop_info_1, sign_loop_info_2, sign_loop_info_3;
    double sign_loop_info_4, sign_loop_info_5, sign_loop_info_6, sign_loop_info_7;
    Eigen::Matrix<double, 8, 1> sign_loop_info;
                  ...
                  &keypoints_num) != EOF)
    {
        ...
        */
        ...
        // 20240917 xjl
        int sign_loop_index = -1;
        Eigen::Matrix<double, 8, 1> sign_loop_info;
        sign_loop_info << sign_loop_info_0, sign_loop_info_1, sign_loop_info_2, sign_loop_info_3, sign_loop_info_4, sign_loop_info_5, sign_loop_info_6, sign_loop_info_7;
        ...
        KeyFrame *keyframe = new KeyFrame(time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image, loop_index, loop_info, sign_loop_index, sign_loop_info, keypoints, keypoints_norm, brief_descriptors);
...
// 20240917 xjl
// sign_loop_info << sign_relative_t.x(), sign_relative_t.y(), sign_relative_t.z(),
//     sign_relative_q.w(), sign_relative_q.x(), sign_relative_q.y(), sign_relative_q.z(),
//     sign_relative_yaw;
void PoseGraph::updateKeyFrameSignLoop(double stamp, Eigen::Matrix<double, 8, 1> &_sign_loop_info)
{
    KeyFrame *kf = SigngetKeyFrame(stamp);
    if (kf == NULL)
    {
        return;
    }
    int _index = kf->index;
    kf->updateSignLoop(_sign_loop_info);
    kf->has_sign_loop = 1;
    // if (abs(_sign_loop_info(7)) < 30.0 && Vector3d(_sign_loop_info(0), _sign_loop_info(1), _sign_loop_info(2)).norm() < 20.0)
    {
        Vector3d w_P_old, w_P_cur, vio_P_cur;
        Matrix3d w_R_old, w_R_cur, vio_R_cur;
        ...
        Vector3d relative_t;
        Quaterniond relative_q;
        relative_t = Vector3d(_sign_loop_info(0), _sign_loop_info(1), _sign_loop_info(2));
        relative_q = Quaterniond(_sign_loop_info(3), _sign_loop_info(4), _sign_loop_info(5), _sign_loop_info(6));
        double shift_sign_yaw;
        Matrix3d shift_sign_r;
        Vector3d shift_sign_t;
        shift_sign_yaw = _sign_loop_info(7);
        shift_sign_r = relative_q.toRotationMatrix();
        shift_sign_t = relative_t;

        // printf("receive sign index %d \n", _index);
        // cout << "sign_relative_t:" << relative_t.x() << "," << relative_t.y() << "," << relative_t.z() << endl;
        // cout << "sign_relative_q:" << relative_q.w() << "," << relative_q.x() << "," << relative_q.y() << "," << relative_q.z() << endl;
        m_sign_drift.lock();
        sign_yaw_drift = shift_sign_yaw;
        sign_r_drift = shift_sign_r;
        sign_t_drift = shift_sign_t;
        m_sign_drift.unlock();
        m_sign_optimize_buf.lock();
        sign_optimize_buf.push(kf->index);
        m_sign_optimize_buf.unlock();
    }
}
