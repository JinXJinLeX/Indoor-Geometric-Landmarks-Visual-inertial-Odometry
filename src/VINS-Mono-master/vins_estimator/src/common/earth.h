...
public:
    ...
    static Eigen::Vector2d meridianPrimeVerticalRadius(double lat) {
        ...
        return {WGS84_RA * (1 - WGS84_E1) / (sqrttmp * tmp), WGS84_RA / sqrttmp};//WGS84_RA为椭球长轴，WGS84_E1为椭球偏心率
    }

    static double RN(double lat) {
        double sinlat = sin(lat);
        return WGS84_RA / sqrt(1.0 - WGS84_E1 * sinlat * sinlat);//WGS84_RA为椭球长轴，WGS84_E1为椭球偏心率
    }
    ...
    static Matrix3d cne(const Vector3d &blh) {
        ...
        // dcm(1,0) = -sinlat*coslon;
        // dcm(1,1) = -sinlat*sinlon;
        // dcm(1,2) = coslat;

        // dcm(2,0) = coslat*coslon;
        // dcm(2,1) = coslat*sinlon;
        // dcm(2,2) = sinlat;
    ...
    static Vector3d global2local(const Vector3d &origin, const Vector3d &global) {
        Vector3d ecef0 = blh2ecef(origin);//转为ecef坐标系
        Matrix3d cn0e  = cne(origin);//到东北天的旋转矩阵
